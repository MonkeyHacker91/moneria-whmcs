...
$initialTab = (!empty($defaultTab) && $defaultTab === 'boleto' && $hasBoleto) ? 'boleto' : ($hasPix ? 'pix' : 'boleto');
?>
...
<div class="moneria-payment-box gen3-boleto <?php echo $themeClass; ?>" data-gen3-boleto data-moneria-box style="<?php echo htmlspecialchars($inlineStyle); ?>">
    ...
    <?php else: ?>
        ...
        <!-- PIX PANE -->
        <?php if ($hasPix): ?>
            <div id="gen3-pane-pix" class="moneria-pane" style="display: <?php echo ($initialTab === 'pix' ? 'block' : 'none'); ?>;">
...
.moneria-payment-box.moneria-theme-light .gen3-boleto__due strong {
  color: #0f172a !important;
}
.moneria-payment-box.moneria-theme-light .gen3-tab-switch__btn {
  border-color: #cbd5e1 !important;
  background: #f1f5f9 !important;
  color: #64748b !important;
}
.moneria-payment-box.moneria-theme-light .gen3-tab-switch__btn.is-active {
...
.moneria-payment-box.moneria-theme-slate .gen3-tab-switch__btn.is-active {
...
.gen3-tab-switch__btn i {
  font-size: 11px !important;
  flex: 0 0 auto !important;
}
.gen3-tab-switch__btn span {
  white-space: nowrap !important;
  text-overflow: ellipsis !important;
  overflow: hidden !important;
...
.gen3-tab-switch__btn:hover {
  color: var(--gb-text) !important;
  background: rgba(255, 255, 255, 0.08) !important;
  border-color: rgba(255, 255, 255, 0.22) !important;
}
.gen3-tab-switch__btn.is-active {
  background: rgba(226, 200, 58, 0.12) !important;
  border-color: rgba(226, 200, 58, 0.45) !important;
...
.gen3-boleto__note strong { color: var(--gb-text); font-weight: 700; }
...
function moneriaSwitchPaymentTab(tab) {
    ...
    var pixTab = document.getElementById('gen3-tab-pix');
    var bolTab = document.getElementById('gen3-tab-boleto');

    if (tab === 'pix') {
        if (pixPane) pixPane.style.display = 'block';
        if (bolPane) bolPane.style.display = 'none';
        if (pixTab) pixTab.classList.add('is-active');
        if (bolTab) bolTab.classList.remove('is-active');
    } else {
        if (pixPane) pixPane.style.display = 'none';
        if (bolPane) bolPane.style.display = 'block';
        if (pixTab) pixTab.classList.remove('is-active');
        if (bolTab) bolTab.classList.add('is-active');
    }
}
window.moneriaSwitchPaymentTab = moneriaSwitchPaymentTab;
window.gen3SwitchPaymentTab = moneriaSwitchPaymentTab;
